...
from Plugins.Extensions.PKT.InfoCenter import headers as hs
...
#        url = 'http://e2.areq.eu.org/ummeteo/version'
#        try:
#            self.updateCB(urlopen(Request(url, headers=hs)).read().decode("utf-8"))
#        except:
#            self.errorUpdate(urlopen(Request(url, headers=hs)).read().decode("utf-8"))
    ...
    def start_meteo(self, id ):
        ...
        try:
            www = urlopen(Request(url, headers=hs)).read()
    ...
    def infoCB(self, html):
        ...
        if m:
            ...
            try:
                www = urlretrieve(url, '/tmp/meteo.png')
                self.pngCB(www)
            except:
                self.error()
        else:
